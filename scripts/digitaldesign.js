...
$(document).ready(function () {
    ...
    const navContainer = $('.digital-design-nav');

    let galleryData = [];

    $.getJSON('/json/digital-design.json', function (data) {
        galleryData = data;

        // Dynamically build nav
        galleryData.forEach((item, index) => {
            ...
            const navItem = $('<a></a>')
                ...
                .on('click', function (e) {
                    e.preventDefault();
                    $('.digital-design-nav a').removeClass('active');
                    $(this).addClass('active');
                    loadGallery(title);
                });

            // First item: add active + load gallery
            if (index === 0) {
    ...
    function loadGallery(title) {
        const entry = galleryData.find(item => item['gallery-title'] === title);
        if (!entry) return;
